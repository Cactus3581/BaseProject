...
@interface BPTagAutoLayoutView()
@property (nonatomic,strong) NSMutableArray *buttonArray;
@property (nonatomic,weak) UIView *backView;
@end

@implementation BPTagAutoLayoutView
    ...
    [self.titlesArray enumerateObjectsUsingBlock:^(NSString *title, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx == 0) {
            [button mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.width.mas_equalTo(width <= self.backView.width ? width:self.backView.width);
            }];
        }else {
