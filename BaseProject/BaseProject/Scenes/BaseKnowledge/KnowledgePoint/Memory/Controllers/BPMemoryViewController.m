...
//    [self testSet];
    ...
    [self func_secondaryPointer]; // 指针传递
    [self superSelf];//self 与super
}
...
- (void)func_secondaryPointer {
    ...
    BPLog(@"%@",sName);
    [self testStatic:sName];
    BPLog(@"%@",sName);
    
    //    [self testAStatic:&(sName)];
    //    BPLog(@"%@",sName);
    
    
    
    BPMemoryModel *model = [[BPMemoryModel alloc] init];
    model.memoryName = @"memoryName_A";

    [self setMemoryModel:model];
    BPLog(@"%@",model.memoryName);    
}

- (void)setMemoryModel:(BPMemoryModel *)model {
    model.memoryName = @"memoryName_B";
    BPLog(@"%@",model.memoryName);
}
...
- (void)testAStatic:(NSString **)name {
    *name = @"b";
    BPLog(@"%@",*name);
}
