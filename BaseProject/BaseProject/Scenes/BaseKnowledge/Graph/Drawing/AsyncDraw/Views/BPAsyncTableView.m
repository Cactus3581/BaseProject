...
@interface BPAsyncTableView() <UITableViewDelegate, UITableViewDataSource>

@end

@implementation BPAsyncTableView{
...
- (void)drawCell:(BPAsyncDrawTableViewCell *)cell withIndexPath:(NSIndexPath *)indexPath{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    BPAsyncDrawTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (cell==nil) {
        cell = [[BPAsyncDrawTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)loadContent{
    ...
    if (self.visibleCells&&self.visibleCells.count>0) {
        for (id temp in [self.visibleCells copy]) {
            BPAsyncDrawTableViewCell *cell = (BPAsyncDrawTableViewCell *)temp;
            [cell draw];
        }
    }
}
...
- (void)removeFromSuperview{
    for (UIView *temp in self.subviews) {
        for (BPAsyncDrawTableViewCell *cell in temp.subviews) {
            if ([cell isKindOfClass:[BPAsyncDrawTableViewCell class]]) {
                [cell releaseMemory];
            }
        }
    }
