...
@interface BPFlowCategoryImageTagView ()<UICollectionViewDataSource, UICollectionViewDelegate>
...
@property (nonatomic, weak) BPFlowCategoryViewCellModel *fromModel;
@property (nonatomic, weak) BPFlowCategoryViewCellModel *toModel;
@end

@implementation BPFlowCategoryImageTagView
...
- (void)bp_initializeUI{
    BPFlowCategoryViewLayout *categoryLayout = [BPFlowCategoryViewLayout new];
    BPFlowCategoryViewProperty *property = [BPFlowCategoryViewProperty new];
    _property = property;
    categoryLayout.property = property;
    BPFlowCollectionView *mainView = [[BPFlowCollectionView alloc] initWithFrame:self.frame collectionViewLayout:categoryLayout];
    _mainView = mainView;
    ...
    [mainView registerClass:[BPFlowCategoryImageTagCell class] forCellWithReuseIdentifier:@"BPFlowCategoryImageTagCell"];
...
- (void)setBackEllipseSize:(CGSize)backEllipseSize{
    ...
    if (!CGSizeEqualToSize(CGSizeZero, backEllipseSize)) {
        for (BPFlowCategoryViewCellModel *model in _data) {
...
- (void)bp_interpolationForItemsWithRatio:(CGFloat)ratio{
    ...
    for (BPFlowCategoryImageTagCell *cell in _mainView.visibleCells) {
        [cell bp_updateCell];
    }
}
...
- (void)bp_updateBottomLineWhenClickedWithIndexPath:(NSIndexPath *)indexPath{
    if (!_bottomLineEable || !_data.count) return;
    BPFlowCategoryViewCellModel *model = _data[indexPath.item];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    BPFlowCategoryImageTagCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BPFlowCategoryImageTagCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //通知代理
    if (_delegate && [_delegate respondsToSelector:@selector(categoryView:didSelectItemAtIndexPath:)]) {
        [_delegate categoryView:self didSelectItemAtIndexPath:indexPath];
    }
    _lastIndexPath = indexPath;
}
