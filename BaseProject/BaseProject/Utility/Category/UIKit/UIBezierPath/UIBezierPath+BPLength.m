...
typedef struct BPBezierSubpath {
    CGPoint startPoint;
...
@implementation UIBezierPath (BPLength)
...
- (void)extractSubpaths:(BPBezierSubpath*)subpathArray
{
...
- (CGPoint)pointAtPercent:(CGFloat)t ofSubpath:(BPBezierSubpath)subpath {
...
- (CGFloat)bp_length {
	...
	BPBezierSubpath subpaths[subpathCount];
...
- (CGPoint)bp_pointAtPercentOfLength:(CGFloat)percent {
    ...
    BPBezierSubpath subpathContainingPoint;
	for (NSUInteger i = 0; i < subpathCount; i++) {
