...
- (UIImage *)bp_MiddleAlignedButtonImageScaleToSize:(CGSize)size
{
...
@implementation UIButton (BPMiddleAligning)
...
- (void)bp_middleAlignButtonWithSpacing:(CGFloat)spacing
{
    ...
    if (imageSize.width > ceilf(maxImageWidth)) {
        ...
        newImage = [self.imageView.image bp_MiddleAlignedButtonImageScaleToSize:CGSizeMake(maxImageWidth, imageSize.height * ratio)];
        imageSize = newImage.size;
    }
    if (imageSize.height > ceilf(maxImageHeight)) {
        CGFloat ratio = maxImageHeight / imageSize.height;
        newImage = [self.imageView.image bp_MiddleAlignedButtonImageScaleToSize:CGSizeMake(imageSize.width * ratio, maxImageHeight)];
        imageSize = newImage.size;
    }
    if (newImage) {
