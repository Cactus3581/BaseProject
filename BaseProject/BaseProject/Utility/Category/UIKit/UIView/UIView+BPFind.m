...
- (id)bp_findSuperViewWithSuperViewClass:(Class)clazz
{
    if (self == nil) {
        ...
        return [self.superview bp_findSuperViewWithSuperViewClass:clazz];
    }
}
/**
...
- (BOOL)bp_findAndResignFirstResponder {
    ...
    for (UIView *v in self.subviews) {
        if ([v bp_findAndResignFirstResponder]) {
            return YES;
        }
    }
...
- (UIView *)bp_findFirstResponder {
    ...
    for (UIView *v in self.subviews) {
        UIView *fv = [v bp_findFirstResponder];
        if (fv) {
            return fv;
        }
    }
 ...
 */
- (UIViewController *)bp_viewController
{
