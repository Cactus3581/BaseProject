...
@implementation UIDevice (BPHardware)
...
+ (NSString *)bp_platformString{
    NSString *platform = [self bp_platform];
...
+ (NSUInteger)bp_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)bp_cpuFrequency {
    return [self bp_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)bp_busFrequency {
    return [self bp_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)bp_ramSize {
    return [self bp_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)bp_cpuNumber {
    return [self bp_getSysInfo:HW_NCPU];
}


+ (NSUInteger)bp_totalMemoryBytes
{
    return [self bp_getSysInfo:HW_PHYSMEM];
}
