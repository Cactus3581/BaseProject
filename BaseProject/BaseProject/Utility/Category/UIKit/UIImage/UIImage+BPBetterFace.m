...
@implementation UIImage (BPBetterFace)

- (UIImage *)bp_betterFaceImageForSize:(CGSize)size
                           accuracy:(BPAccuracy)accurary;
{
    NSArray *features = [UIImage _bp_faceFeaturesInImage:self accuracy:accurary];
    ...
    if ([features count]==0) {
        ...
        return [self _bp_subImageForFaceFeatures:features
                                         size:size];
    }
}

- (UIImage *)_bp_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
...
+ (NSArray *)_bp_faceFeaturesInImage:(UIImage *)image accuracy:(BPAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == BPAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;
