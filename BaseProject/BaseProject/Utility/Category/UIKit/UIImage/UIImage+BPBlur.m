...
- (UIImage *)bp_lightImage
{
    ...
    return [self bp_blurredImageWithSize:CGSizeMake(60, 60) tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage *)bp_extraLightImage
{
    ...
    return [self bp_blurredImageWithSize:CGSizeMake(40, 40) tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage *)bp_tintedImageWithColor:(UIColor *)tintColor
{
    ...
    return [self bp_blurredImageWithSize:CGSizeMake(20, 20) tintColor:effectColor saturationDeltaFactor:-1.0 maskImage:nil];
}
...
{
    return [self bp_blurredImageWithSize:CGSizeMake(blurRadius, blurRadius)];
}
...
{
    return [self bp_blurredImageWithSize:blurSize tintColor:nil saturationDeltaFactor:1.0 maskImage:nil];
}
...
//| ----------------------------------------------------------------------------
- (UIImage *)bp_blurredImageWithSize:(CGSize)blurSize tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(UIImage *)maskImage
{
...
#define ENABLE_TINT                     1
    ...
    {
        BPLog(@"*** error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage)
    {
        BPLog(@"*** error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage)
    {
        BPLog(@"*** error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (hasBlur || hasSaturationChange)
    {
        ...
        if (e != kvImageNoError)
        {
            BPLog(@"*** error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", e, self);
        ...
        if (hasBlur)
        {
            CGFloat radiusX = [self bp_gaussianBlurRadiusWithBlurRadius:blurSize.width * inputImageScale];
            CGFloat radiusY = [self bp_gaussianBlurRadiusWithBlurRadius:blurSize.height * inputImageScale];
        ...
        if ( (effectCGImage = vImageCreateCGImageFromBuffer(inputBuffer, &format, &bp_cleanupBuffer, NULL, kvImageNoAllocate, NULL)) == NULL ) {
...
//
- (CGFloat)bp_gaussianBlurRadiusWithBlurRadius:(CGFloat)blurRadius
{
...
//
void bp_cleanupBuffer(void *userData, void *buf_data)
{ free(buf_data); }
