...
- (NSArray *)bp_getImgs
{
    ...
    for (int i = 0; i < [self bp_nodeCountOfTag:@"img"]; i++) {
...
{
    NSString * jsString = [NSString stringWithFormat:@"document.body.style.backgroundColor = '%@'",[color bp_webColorString]];
...
- (void)bp_setFontColor:(UIColor *)color withTag:(NSString *)tagName
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          nodes[i].style.color = '%@';}", tagName, [color bp_webColorString]];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
/// 改变指定标签的字体大小
