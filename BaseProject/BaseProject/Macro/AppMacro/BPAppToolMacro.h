...
/**
 自定义NSLog
 使用 : BPLog("你好%@",@"cactus")，第一个"前面无需添加@,添加了也无所谓
 解释 : 将上面的打印换成NSLog可得到 NSLog((@"[文件名:%s]\n" "[函数名:%s]\n" "[行号:%d] \n" "你好%@"), __FILE__, __FUNCTION__, __LINE__, @"str");  fmt用于替换为我们的输出格式字符串，__FILE__宏在预编译时会替换成当前的源文件名,__LINE__宏在预编译时会替换成当前的行号,__FUNCTION__宏在预编译时会替换成当前的函数名称,__VA_ARGS__是一个可变参数的宏,使得打印的参数可以随意，而##可以在__VA_ARGS__的参数为0的时候保障编译正确;
...
#ifdef DEBUG
#define BPDLog(_var) ({ NSString *name = @#_var; NSLog(@"变量名 = %@，类型 = %@，对象地址 = %p，指针地址 = %p，值 = %@，引用计数 = %d", name, [_var class], _var, &(_var), _var, (int)(_var ? CFGetRetainCount((__bridge  CFTypeRef)(_var)):(0))); })
#else
#define BPDLog(...)
#endif
